...
class GoalCommentCreateSerializer(serializers.ModelSerializer):
    ...
    def validated_goal(self, value: Goal) -> Goal:
...
class GoalCommentSerializer(serializers.ModelSerializer):
    ...
    def validated_goal(self, value: GoalCategory) -> GoalCategory:
        if self.context['request'].user != value.user:
            raise PermissionDenied
        return value
...
class BoardSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance: Board, validated_data: dict):
        with transaction.atomic():
            BoardParticipant.objects.filter(board=instance).exclude(user=self.context['request'].user).deleted()
